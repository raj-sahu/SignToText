...
def handtracking():
    ...
    out = cv2.VideoWriter('outputWebCan.mp4', fourcc, 5, size)
        ...
        if points is not None:
            ...
            for connection in connections:
                ...
                if connection in palm:
                    cv2.line(mask, (int(x0), int(y0)),
                             (int(x1), int(y1)), ConnectionColour, THICKNESS*5)

                else:
                    cv2.line(mask, (int(x0), int(y0)),
                             (int(x1), int(y1)), ConnectionColour, THICKNESS*3)
